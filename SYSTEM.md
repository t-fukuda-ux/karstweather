...
**F は両モデルの平均、V は ECMWF 由来**。2026-09-17までのカメラ実測85件との比較で、前夜予報のVは規定版が実測4段階とほぼ無相関（Pearson -0.02、AUC 0.53）、EC版は明確な相関（Pearson 0.45、AUC 0.72）を示したため変更した。ECMWFは予報視程が全欠測だが、V_visは計算に使っておらず、V_summitは算出できる。混成である旨は凡例に明記している。
