...
**1. `V_summit` は感度100%。ただし空振り率が測れていない**

前夜発表の `forecast-history` と翌朝のカメラ実測、照合9組（リード12〜38時間）で、**`V_summit = 0.000` が9組すべて、実測も9組すべて霧**。ただし**非ゼロの予報が1件もない**ため、「常に霧と言えば100%当たる」状態と区別がつかない。**晴天日のデータが揃うまで的中率とは呼べない。**
