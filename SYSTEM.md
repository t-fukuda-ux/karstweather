...
**⚠ 2026-09-10 追記: カメラ実測36時刻での検証では、この `V_summit` は霧を区別できなかった**（展望ありの11時刻すべてが `summit_in_layer` に落ち、空振り100%）。**原因は式ではなく best_match の湿度**で、同じ式を ECMWF の湿度で計算すると正解率83%まで上がる。詳細は12節「カメラによる霧予報の検証」を参照。雲海指数としての妥当性は別問題だが、`V_summit` を霧の指標として使う場合は**湿度の入力を EC版に替えること**。なお有効なのは V≤0.50 の切り方で、0.00/0.15 を強い減点とする現行の重みづけとは向きが違う。
...
| **霧なし** | ≥ 3.82 | 奥まで抜けている | あり |

閾値は 2026-09-10 に画像を目視して暫定決定（昼間29枚）。境界に該当画像のない空白があり、閾値として安全: **1.83｜2.65** と **3.51｜3.82**。
...
**⚠ この閾値は昼間ゲートが前提。** 夜間のセンサーノイズは near_lc **3.80〜3.92** に密集しており、霧なしの閾値3.82とほぼ重なる。ゲートなしでは毎晩「霧なし」と判定される（旧指標の比が毎晩0.94だったのと同じ罠）。`near_mean > 100` で夜間(約28)と薄暮(38〜75)は完全に切れる。
...
**⚠ 原因は式の構造ではなく、入力モデルの湿度。** RHの閾値を動かしても直らない（規定版は88%閾値の64%が最良で、ベースライン69%に届かない）。しかし **ECMWF の湿度に入れ替えると分離する。**
...
**式の構造自体は妥当。** 同じ `V_summit` の式を EC版の湿度で計算すると、**V≤0.50 を霧とする判定で正解率83%（+13.9pt）**まで上がる（捕捉24/25・空振り5/11）。上下を別々に見る構造を素の最小RH（75%どまり）より活かせている。
...
**ベースライン（常に「霧」と予報）の正解率は 62%（18/29）。これに勝てない閾値は意味がない。**
...
**⚠ 検証の設計について。** この期間は湿度が高く霧の時間帯が非常に多かったため、`V_summit` も規定版・平均版の低層雲% も**一度も「霧なし」と言う機会がなかった**。この状態では「常に霧と言えば100%当たる」と区別がつかない。**正例ばかりのデータで指数の良し悪しを論じないこと**（見逃しは測れるが、空振りは測れない）。指数を評価するには晴天日が要る。
...
- **`capture-mezuru.ps1` を局所コントラスト方式に置き換える**（最優先）。現在の `far_sd`/`far_over_near` は上記の通り誤った指標のままで、CSVに記録され続けている。`*_lc` 列とラベル列を追加し、`far_over_near` は記録のみに降格する
- **カメラタスクの `WakeToRun` を有効化する。** 現在 `WakeToRun=False` / `LogonType=Interactive` のためPC停止中は動かない。2026-09-09 22:50〜09-10 10:50 に12時間の欠測が発生し、**落ちたのが朝5〜9時という霧の最重要時間帯だった**。「記録が目的なので欠測があっても支障はない」という当初の想定は、朝に関しては成り立たない
- **晴天日のカメラ実測を溜める。** 現状は霧なしが3時刻しかなく、`V_summit` も規定版・平均版の低層雲% も空振り率が測れていない。指数の評価はここが埋まってから
...
- **4区分（濃霧/薄霧/靄/霧なし）を `capture-mezuru.ps1` のラベル列に実装する。** 靄を霧なし側に寄せる線引きが検証の成否を分けた（3区分では空振りが測れなかった）
