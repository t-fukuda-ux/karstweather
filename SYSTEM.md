...
- **霧の見通し（平均版のみ、2026-09-25〜）**: 警報バナーの下・毎時テーブルの上に、今日と明日を 朝(5-10時)・昼(11-15時)・夕(16-20時) に分けて3段階で表示。低層雲（平均版）の時間帯平均が 60%以上＝●霧が濃い見込み／30〜60%＝◐霧が出たり晴れたりしそう／30%未満＝○霧の心配はほぼなし。今日の終わった時間帯は薄く表示。時刻単位の霧の出入りは予報で当てられないため時間帯でまとめる（`camera/fog-verification-20260923.md`）。関数は `lowcloud_common.ps1` の `Get-FogOutlook` / `Render-FogOutlookHtml`
