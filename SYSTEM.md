...
`publish.ps1` と Actions はいずれも `git add` の対象を**上記の追跡5ファイルに明示限定**している。新しい生成物を公開したい場合は両方に追加が必要。
...
| 雲海あり・展望良好 | F高 × V高 |
| 雲海あり・展望不良 | F高 × V中 |
| **雲海あり・現地も霧の疑い** | F高 × V低。現地に行っても見えない可能性 |
| 雲海あり・雲頂不確実 | 雲頂の推定範囲が H_S を跨ぐ |
| 雲海あり・展望は判定材料不足 | V_summit を算出できない |
| 雲海の条件なし / 降水あり / 判定不能 | — |
...
- 1版の生成に失敗しても、成功した版は commit・公開される。**失敗メールが届くのは全版失敗時のみ**（一時的な取得失敗のたびに通知が飛ぶと本当の異常が埋もれるため）
