...
GitHubのscheduledイベントが欠落しても、サーバーPCから毎時20分に`workflow_dispatch`を送り、天気予報を更新する。宿泊管理を最優先とし、このトリガーは予報生成・git操作を行わない。

※ 2026-09-23 から、このPCでの git 操作そのものは許可した（福田さん判断）。トリガーとは別に、カメラ記録の2ファイルだけを `camera/push-camera-data.ps1` が1日2回 commit・push する（SYSTEM.md 12節）。

## 処理

1. 多重起動防止mutexを取得する。前回分が動作中なら終了する。
2. 自プロセスを低優先度へ変更する。
3. 公開ページが25分以内に更新済みなら、その時点で正常終了する。
...
7. 回線断などで失敗した場合、**スクリプト内で**10分待って最大3回まで試す。試すたびに公開ページを見直し、更新済みならそこで終える。タスクスケジューラの RestartCount は終了コード1では働かないため頼らない（2026-09-23 に判明）。
...
- このトリガーはAPIデータ取得、HTML生成、git clone/pull/pushをしない。
- 同時実行は1つだけ。再試行を含めて60分で強制終了する。
