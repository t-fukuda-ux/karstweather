...
function Get-UnkaiLabHtml {
    ...
    foreach ($model in $ByModel.Keys) {
        ...
        [void]$sb.AppendLine('</table></div>')
        [void]$sb.AppendLine(("<p class=""note""><b>V = min(V_top_fog, V_summit)</b>（V_vis は2026-09-07から記録のみ）。適用できた補正だけの最小値をとり、すべて適用できなければ「--」です。<b>補正を適用しないことと「良好(1.0)」は区別</b>しており、湿潤層を検出できない場合や上端が不明な場合を 1.0 にはしていません。<br>" +
            ...
            "0／0.15／0.50／1.0 はいずれも仮の係数です。<b>0 は「確実に霧」ではなく、濃い霧の可能性を重く見た暫定的な強い減点</b>です。上下の湿度だけでは 0.15 と 0.50 の差を物理的に確定できません（直上だけ湿っていても、層の下端が展望地点より下にある可能性があります）。<br>" +
